...
def apply_ken_burns(clip, target_w, target_h, zoom_to=1.04):
    """Slow progressive zoom from 1.0x to zoom_to over clip duration.
    Clip must already be cropped to target_w x target_h before calling.
    Stops at fade to black — applied to main clip only, not outro."""
...
def build_caption_panel(video_width, video_height, duration):
    """Leather panel with HUD accent for caption background.
    Warm dark leather base + cool blue-grey glass overlay + amber accent line."""
    ...
    leather = (ColorClip(size=(video_width, panel_h), color=(48, 26, 12))
               .set_opacity(0.83)
               .set_position((0, panel_y))
               .set_duration(duration))

    # Cool blue-grey glass overlay — HUD feel on top of leather
    hud_glass = (ColorClip(size=(video_width, panel_h), color=(22, 30, 50))
                 .set_opacity(0.22)
                 .set_position((0, panel_y))
                 .set_duration(duration))

    # Amber/gold accent line at top edge of panel
    accent = (ColorClip(size=(video_width, 3), color=(162, 108, 36))
              .set_opacity(0.88)
              .set_position((0, panel_y))
              .set_duration(duration))

    return [leather, hud_glass, accent]


def build_caption_clips(setup_text, lines, voice_duration, video_width, video_height):
    """Build caption clips for setup text + line 1 + line 2.
    All three captioned in sync with voice. Leather panel positioned."""
    caption_clips = []

    # Build ordered list: setup first, then lines
    all_items = []
    if setup_text:
        # Setup text: pause_after True so voice has a beat before reacting
    ...
    # Position: inside the leather panel
    panel_h = 270
    panel_y = video_height - panel_h
    text_y = panel_y + 55  # vertically centered within panel with padding
...
def assemble_video(image_path, audio_path, lines, output_path, setup_text=None,
    ...
    """
    New structure (no setup card):
    1. Face at full brightness from frame 1
    2. Voice starts immediately (setup text + line1 + line2)
    3. Captions: setup text, line 1, line 2 — all on leather/HUD panel
    4. Slow Ken Burns zoom throughout main clip
    5. Post-voice hold: clean face, no panel, zoom continues
    6. Fade to black → outro
    """
    ...
    # ── Load and frame image (face zoom + crop) ─────────────────────────────
    ...
    # ── Duration: voice + post hold ─────────────────────────────────────────
    post_hold = 2.2
    total_duration = voice_duration + post_hold
    image_clip = image_clip.set_duration(total_duration)

    # ── Ken Burns slow zoom (face to face, 1.0x → 1.04x) ───────────────────
    print('[Assemble] Applying Ken Burns zoom...')
    image_clip = apply_ken_burns(image_clip, target_width, target_height, zoom_to=1.04)

    # ── Leather panel + HUD (visible during voice only) ─────────────────────
    panel_layers = build_caption_panel(target_width, target_height, voice_duration)

    # ── Caption clips (setup + line1 + line2, voice-synced) ─────────────────
    lines_parsed = lines
    caption_clips = build_caption_clips(setup_text, lines_parsed, voice_duration, target_width, target_height)
    ...
    # Black gap between main clip and outro

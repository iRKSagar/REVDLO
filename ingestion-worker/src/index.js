...
function applyKeywordScoring(title, baseScore) {
  ...
  let score = baseScore;
  if (BOOST_KEYWORDS.some(k => lower.includes(k))) score = score * 2;
  if (PENALIZE_KEYWORDS.some(k => lower.includes(k))) score = score * 0.5;
...
async function fetchGoogleTrends() {
  ...
  for (const feed of feeds) {
    try {
      const res = await fetch(feed.url, { headers: { 'User-Agent': 'MrOldverdict-Bot/1.0' } });
      if (!res.ok) continue;
...
async function runIngestion(env) {
  const results = {
    fetched: 0,
    stored: 0,
    by_source: { reddit: 0, google_trends: 0, hacker_news: 0 },

...
THE HOOK SCENE RULE
Every script must include a hook_scene. This is a visual brief for a philosophical, cinematic image that represents the modern behavior described in the setup — before Mr. Oldverdict weighs in on it.

The hook_scene is not Mr. Oldverdict. It is the world he is observing.

It must be imageable, specific, and evocative. It describes a single visual moment — an object, an environment, a symbolic scene — that captures the irony or emptiness or absurdity of the modern behavior. The visual treatment should feel old, heavy, and painterly even when the subject is modern. That contrast is the hook.

Three types that work well:
Object isolation: a single modern object in dramatic isolation. A glowing phone face-up on a dark wooden table. A productivity timer sitting next to an untouched meal.
Environmental irony: the modern behavior placed in a vast or timeless setting that makes it look small. A person in business attire sitting alone in an enormous empty field, typing.
Symbolic substitution: replace the modern thing with an ancient equivalent that reveals what it actually is. An hourglass where the sand has been replaced with notification icons. A set of scales weighing a diploma against a loaf of bread.

The hook_scene must not describe people directly — no faces, no portraits. Environments, objects, symbols, light. The viewer should feel something before a word is spoken.

Keep it to one or two sentences. Specific beats vague. Imageable beats poetic.
...
OUTPUT FORMAT - return valid JSON only, no markdown, no extra text
{
  ...
  "scene": "One line visual direction for the main atmospheric scene image.",
  "hook_scene": "One to two sentences describing a philosophical cinematic image of the modern world being observed. No people. Objects, environments, symbols, light. Specific and imageable.",
  "lines": [
...
async function storeScript(supabaseUrl, supabaseKey, topicId, category, rawTopic, script, relatedScripts) {
  const response = await fetch(`${supabaseUrl}/rest/v1/scripts`, {
    ...
    body: JSON.stringify({
      ...
      raw_topic:     rawTopic,
      scene:         script.scene,
      hook_scene:    script.hook_scene || null,
      setup:         script.setup,
      lines:         script.lines,
      prop:          script.prop,
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Fallback hook_scene if GPT omits it
      if (!script.hook_scene) {
        script.hook_scene = `A symbolic still life representing the modern behavior: ${rawTopic}. No people. Dramatic side lighting, dark background, painterly quality.`;
      }
